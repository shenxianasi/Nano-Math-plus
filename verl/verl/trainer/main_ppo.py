...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, is_train=True):
    ...
    if not is_train:
        subset_size = data_config.get("val_subset_size", None)
        if subset_size is not None:
            subset_size = int(subset_size)
            if subset_size > 0 and subset_size < len(dataset):
                seed = int(data_config.get("val_subset_seed", 1))
                rng = random.Random(seed)
                indices = rng.sample(range(len(dataset)), subset_size)
                indices.sort()
                if hasattr(dataset, "dataframe"):
                    dataset.dataframe = dataset.dataframe.select(indices)
                    print(f"Validation subset size: {len(dataset)} (seed={seed})")
                else:
